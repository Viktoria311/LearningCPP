...
bool is_there_a_winner(char** my_arr, int quantity, int  rows, int columns)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		for (int j = 0; j < columns; ++j)
			...
			for (int m = 1; m < quantity && j + m < rows; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
	...
	for (int i = 0; i < rows; ++i)
	{
		for (int j = 0; j < columns; ++j)
			...
			for (int m = 1; m < quantity && i + m < rows; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
				if (my_arr[i][j] != '*' && my_arr[i][j] == my_arr[i + m][j]) ++count;

		if (count == quantity - 1) return true;
		count = 0;
	}
	//ïðîâåðêà ïî äèàãîíàëè ÑËÅÂÀ ÍÀÏÐÀÂÎ È ÑÂÅÐÕÓ ÂÍÈÇ
	for (int j = 0; j < columns; ++j)
	{
		for (int i = 0; i < rows; ++i)

			for (int m = 1; m < quantity    &&  i + m < rows &&  j + m < columns; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
	...
	for (int j = 0; j < columns; ++j)
	{
		for (int i = rows - 1; i >= 0; --i)
			...
			for (int m = 1; m < quantity && i - m >= 0 && j + m < columns; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
				if (my_arr[i][j] != '*' && my_arr[i][j] == my_arr[i - m][j + m]) ++count;
	...
	//ïðîâåðêà ïî äèàãîíàëè ÑÏÐÀÂÀ ÍÀËÅÂÎ È ÑÂÅÐÕÓ ÂÍÈÇ
	for (int j = columns - 1; j >= 0; --j)
	{
		for (int i = 0; i < rows; ++i)

			for (int m = 1; m < quantity && i + m < rows  && j - m >= 0; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
				if (my_arr[i][j] != '*' && my_arr[i][j] == my_arr[i + m][j - m]) ++count;
	...
	//ïðîâåðêà ïî äèàãîíàëè ÑÏÐÀÂÀ ÍÀËÅÂÎ È ÑÍÈÇÓ ÂÂÅÐÕ
	for (int j = columns - 1; j >= 0; --j)
	{
		for (int i = rows - 1; i >= 0; --i)

			for (int m = 1; m < quantity && i - m >= 0  && j - m >= 0; ++m)//öèêë ïðîâåðêè îäèíàêîâûõ ýëåìåíòîâ ïîäðÿä
				if (my_arr[i][j] != '*' && my_arr[i][j] == my_arr[i - m][j - m]) ++count;
